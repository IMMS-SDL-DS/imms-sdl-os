...
> 실험팀 공유 -> 두 자료(고체 전구체 리스트, Run 기록 시트)를 반영한 설계.
...
지금 스키마의 `MaterialUsage`는 **"이번 Task에 실제로 뭘 얼마나 썼는지"**만 기록한 것.
(`material_name`, `target_mass_mg`, `actual_mass_mg` 등). 근데 실험팀이 보낸 표는
그것과는 다른 정보 — **"이 랩에 어떤 물질이 있고, 어디서 샀고, 어디에 보관돼
있는지"**를 담은 **재고 카탈로그**. 별도 모델로 분리:
...
지금 `MaterialUsage.material_name`은 그냥 문자열 ("ZrOCl2" 같은). 두 가지 방식 중 선택 필요:
...
→ **B를 추천**: 표에 이미 CAS 번호까지 있는 걸 보면 물질 관리를 꽤 엄밀하게 하고 있다는 뜻이라, 이름 매칭보다 ID 참조가 나중에 문제가 덜 생김.
...
- **지금 당장은** 자동화된 Task/Action 기록이 아니라, **사람이 손으로 한 실험 결과를 사후에 시스템에 입력**하는 흐름일 가능성 높음
- **"Auto"와 "Manual" 실험을 같은 스키마로 함께 관리**해야 한다는 뜻 — 자동화가 완성되기 전까지는 Manual 기록이 훨씬 많을 것
...
Auto 실험은 `build_solid_dosing_actions()`로 16단계가 자동 생성되지만, Manual 실험은 로봇이 안 움직이니 Action 시퀀스가 의미를 잃음.
-> 두 가지 선택지:
...
→ 이건 **실험팀과 확인이 필요한 부분**. 어느 쪽이든 지금 스키마(Task.actions가 선택적 필드)로 둘 다 커버 가능하긴 함
