...
## Device 상태 관리 (idle/busy 락)

여러 실험/Task가 같은 장비(Opentron Flex, MultiDose 등)를 공유하므로,
동시에 두 Task가 같은 장비를 쓰려 하면 물리적 충돌이 생길 수 있다 (OCTOPUS 논문의
masking table 개념). `src/database/mongo_client.py`에 이를 위한 3개 함수가 있다:

- `register_device_if_missing()` — 장비가 없으면 idle로 새로 등록. 이미 있으면
  건드리지 않음 (`$setOnInsert`) — 다른 Task가 busy로 표시해둔 걸 실수로 안 되돌림.
- `try_acquire_device()` — 장비가 정확히 "idle"일 때만 "busy"로 바꾸고 `True` 반환.
  이미 busy면 아무것도 안 바뀌고 `False` 반환. MongoDB의 `find_one_and_update`가
  "확인+변경"을 하나의 원자적 연산으로 처리해주므로, 두 Task가 동시에 호출해도
  반드시 하나만 성공한다 (경쟁 상태race condition 방지).
- `release_device()` — 다시 idle로 되돌림. 성공/실패/예외와 무관하게 항상 호출돼야
  하므로 `try/finally`로 감싸서 사용한다.

`src/pipeline/dispense_command.py`의 `run_dispense_command()`가 이 패턴을 실제로 씀:
1. `dev_multidose` 장비를 등록(없으면)
2. `try_acquire_device()`로 잠금 시도 → 실패하면 로봇 호출 없이 `status="held"`로 즉시 반환
3. 성공하면 로봇 실행 → `finally` 블록에서 반드시 `release_device()` 호출
...
- [x] Device 상태(idle/busy) 실시간 갱신 로직 구현 (`try_acquire_device`/`release_device`)
